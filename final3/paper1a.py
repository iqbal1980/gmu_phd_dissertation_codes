...
class PericyteModel:
    def __init__(self):
        ...
        # Baseline parameters
        self.diameter_baseline = 4.0  # Baseline diameter (µm)
        self.cAMP_baseline = 1.0

        # α2 receptor parameters
        self.Kd_alpha2 = 0.1
        self.tau_alpha2 = 3.0  # Faster cAMP dynamics
        self.max_cAMP_inhibition = 0.98  # Stronger cAMP inhibition

        # Contractility parameters
        self.tau_contraction = 10.0  # Faster contraction dynamics
        self.max_constriction = 0.65  # Increased to match ~40% constriction
        self.K_cAMP = 0.5
        ...
        # Agonist parameters
        self.efficacy_clonidine = 1.0  # Full agonist
        self.efficacy_xylazine = 0.9  # Slightly lower efficacy
    ...
    def alpha2_activation(self, NA, atipamezole_conc=0):
        if atipamezole_conc > 0:
            return 0  # Fully block α2 activation
        effective_NA = NA
    ...
    def run_simulation(self, condition='NA', NA_concentration=2.0):
        diameter = np.ones(len(self.t)) * self.diameter_baseline
        cAMP = np.ones(len(self.t)) * self.cAMP_baseline
        NA = np.zeros(len(self.t)) + 0.1  # Tonic NA release
        ...
        if condition == 'NA':
            NA[stim_start:stim_end] = NA_concentration
        elif condition == 'NA+atipamezole':
            NA[stim_start:stim_end] = NA_concentration
            atipamezole[stim_start:stim_end] = 1.0
        elif condition == 'clonidine':
            NA[stim_start:stim_end] = NA_concentration * self.efficacy_clonidine
        elif condition == 'xylazine':
            NA[stim_start:stim_end] = NA_concentration * self.efficacy_xylazine
        elif condition == 'phenylephrine':
            NA[stim_start:stim_end] = 0  # No effect via α2 pathway
    ...
    def plot_results(self, results_dict):
        ...
        axes[0].legend()
        axes[0].axvspan(200, 600, color='gray', alpha=0.2, label='Drug application')

        for condition, (na, camp, diam) in results_dict.items():
            axes[1].plot(self.t, camp, color=colors[condition])
        axes[1].set_ylabel('cAMP (norm.)')
        axes[1].set_title('cAMP Response')
        axes[1].axvspan(200, 600, color='gray', alpha=0.2)
        ...
        axes[2].axvspan(200, 600, color='gray', alpha=0.2)
