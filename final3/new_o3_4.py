...
###############################################################################
# 2. DETAILED SINGLE-CELL MODEL PARAMETERS (for the 15 states)
###############################################################################
# (These parameters come from our corrected ion‐channel and Ca²⁺ handling model.)
...
###############################################################################
# We have 15 states per cell, arranged as follows:
...
# IP3R state fractions (for one channel type)
...
tot = x000_initial + x100_initial + x010_initial + x001_initial + x110_initial + x101_initial + x011_initial + x111_initial
x000_initial /= tot;  x100_initial /= tot;  x010_initial /= tot;  x001_initial /= tot
x110_initial /= tot;  x101_initial /= tot;  x011_initial /= tot;  x111_initial /= tot

# RNA scaling factors (for scaling channel conductances)
...
###############################################################################
# 4. DEFINE INDEX CONSTANTS FOR THE 15 STATES
###############################################################################
STATE_V = 0
STATE_CAI = 1
STATE_ATP = 2
...
###############################################################################
# 5. VECTORIZED REACTION FUNCTIONS (operating on arrays, shape=(N_cells,))
###############################################################################
...
def vectorized_reaction(U, t, params):
    ...
    # Ion currents (using a simplified corrected version)
...
###############################################################################
# 6. PRECOMPUTED SPATIAL LAPLACIAN FOR VOLTAGE DIFFUSION
###############################################################################
...
###############################################################################
# 7. STIMULATION (INJECTION) PARAMETERS
###############################################################################
stim_cell_start = 40
stim_cell_end = 60
...
###############################################################################
# 9. FULL REACTION-DIFFUSION ODE SYSTEM (VECTORIZED)
###############################################################################
...
def f_full(t, U_flat, params):
    ...
    dU_reac = vectorized_reaction(U, t, params)  # reaction term, shape (N_cells,15)
    V = U[:, 0]
    diff_V = D * (L.dot(V))
...
###############################################################################
# 10. TIME DISCRETIZATION AND SOLVER CALL
###############################################################################
simulation_duration = 600  # ms
time_points = 1000
t_eval = np.linspace(0, simulation_duration, time_points)

# Initial condition for all cells (shape: (N_cells, 15))
...
plt.title('Membrane Potential in Selected Cells (Vectorized Reaction–Diffusion)')
plt.legend()
plt.grid(True)
plt.show()
