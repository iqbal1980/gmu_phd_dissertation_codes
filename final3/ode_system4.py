...
def HK_deltas_vstim_vresponse_graph_modified_v2(ggap, Ibg_init, Ikir_coef, cm, dx, K_o, I_app_val, t_span, dt, Ng):
    y0 = np.ones(Ng) * (-33)  # Initial condition
    t = np.arange(t_span[0], t_span[1] + dt, dt)  # Fixed time points with controlled time step
...
dt = 0.1
Ng = 200
HK_deltas_vstim_vresponse_graph_modified_v2(ggap=20.008702984240095, Ibg_init=Ibg_init_val, Ikir_coef=0.9, cm=11.0, dx=1, K_o=4.502039575569403, I_app_val=-70, t_span=t_span, dt=dt, Ng=Ng)