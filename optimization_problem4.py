...
def objective(ggap, Ibg_init, Ikir_coef, dt, cm, a, dx, F, R, K_o):
...
for i in range(n_iterations):
    proposed_params = [s.rvs()[0] for s in space]
    print("Proposed parameters:", proposed_params)
    loss = objective(*proposed_params)
    X_train.append(proposed_params)
    y_train.append(loss)
    X_normalized = scaler.fit_transform(np.array(X_train))
    print(f"Training neural network for iteration {i + 1}/{n_iterations}:")
    nn_model.fit(X_normalized, np.array(y_train), epochs=100, verbose=1, callbacks=[PrintTrainingLoss()])
    #nn_model.fit(X_normalized, np.array(y_train), epochs=100, verbose=0)

best_parameters = X_train[np.argmin(y_train)]
