...
Bm = 111.0     # Total mobile buffer (μM)
aKm = 0.123    # Mobile buffer dissociation constant
...
def calculate_buffering_factors(Ca_i, Ca_ER):
   """
   Calculate buffering factors including mobile buffer from T-cell model
   """
   # Cytosolic buffering including both stationary and mobile buffers
   beta_cyt = 1.0 / (1.0 + (Bscyt * aKscyt) / ((aKscyt + Ca_i)**2) + 
                     (Bm * aKm) / ((aKm + Ca_i)**2))
   
   # ER buffering including both stationary and mobile buffers
...
def calculate_currents(V, Ca_i, atp, dpmca, Ca_ER, x110, params):
   ...
   # SERCA flux (keeping current implementation)
...
def model(t, y, params):
   ...
   # Calculate buffering factors with enhanced mobile buffer
   ...
   # Calcium ODEs with buffering factors
...
def plot_results_with_two_figures(sol, params):
   ...
   # Calculate buffering factors over time
   ...
   # Figure 1: Membrane Potential, Calcium Concentrations, IP3R States, and Buffering
   fig1, axs1 = plt.subplots(6, 1, figsize=(12, 30))  # Added one more subplot for buffering
   ...
   axs1[4].set_ylabel('Buffer Factor', fontsize=12, fontweight='bold')
   axs1[4].legend()
   axs1[4].grid(True)
   ...
   axs1[5].set_ylabel('PMCA\nModulation', fontsize=12, fontweight='bold')
   axs1[5].grid(True)

   fig1.tight_layout()
   plt.show()       

   # Figure 2: Currents, Fluxes, and Gating Variables
