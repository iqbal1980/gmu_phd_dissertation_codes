...
# Normalize IP3R2 states (duplicate IP3R1 as placeholder)
x0002_initial = 0.27
x1002_initial = 0.039
x0102_initial = 0.29
x0012_initial = 0.17
...
def plot_results_with_two_figures(sol, params):
    ...
    for i in range(len(sol.t)):
        Ca_i[i] = max(Ca_i[i], 1e-12)
        Ca_ER[i] = max(Ca_ER[i], 1e-12)
        ...
        (I_Kir61[i], I_TRPC1[i], I_CaCC[i], I_CaL[i], I_leak[i], d_inf[i], f_inf[i], I_PMCA[i],
         J_SERCA[i], J_ER_leak[i], I_NCX[i], J_IP3R[i], J_RyR[i]) = calculate_currents(
             V[i], Ca_i[i], params['initial_atp'], dpmca[i], Ca_ER[i], x110[i], x1102[i], IP3[i], params)

    fig1, axs1 = plt.subplots(9, 1, figsize=(12, 45))
    fig1.suptitle('Cell Dynamics - Phase 4 with IP3R2 (All IP3R States)', fontsize=16, fontweight='bold')
    ...
    # IP3R1 x110
    axs1[3].plot(sol.t, x110, label='x110 (IP3R1)', color='purple', linewidth=2)
    axs1[3].set_title('IP3R1 x110 State', fontsize=14, fontweight='bold')
    axs1[3].grid(True)
    axs1[3].legend()

    # All other IP3R1 states
    axs1[4].plot(sol.t, x000, label='x000', linestyle='--')
    axs1[4].plot(sol.t, x100, label='x100', linestyle='--')
    axs1[4].plot(sol.t, x010, label='x010', linestyle='--')
    axs1[4].plot(sol.t, x001, label='x001', linestyle='--')
    axs1[4].plot(sol.t, x101, label='x101', linestyle='--')
    axs1[4].plot(sol.t, x011, label='x011', linestyle='--')
    axs1[4].plot(sol.t, x111, label='x111', linestyle='--')
    axs1[4].set_title('All Other IP3R1 States', fontsize=14, fontweight='bold')
    axs1[4].grid(True)
    axs1[4].legend(ncol=4)

    # All IP3R2 states
    axs1[5].plot(sol.t, x0002, label='x0002', linestyle='--')
    axs1[5].plot(sol.t, x1002, label='x1002', linestyle='--')
    axs1[5].plot(sol.t, x0102, label='x0102', linestyle='--')
    axs1[5].plot(sol.t, x0012, label='x0012', linestyle='--')
    axs1[5].plot(sol.t, x1102, label='x1102', linestyle='--')
    axs1[5].plot(sol.t, x1012, label='x1012', linestyle='--')
    axs1[5].plot(sol.t, x0112, label='x0112', linestyle='--')
    axs1[5].plot(sol.t, x1112, label='x1112', linestyle='--')
    axs1[5].set_title('All IP3R2 States', fontsize=14, fontweight='bold')
    axs1[5].grid(True)
    axs1[5].legend(ncol=4)

    axs1[6].plot(sol.t, beta_cyt, label='Cytosolic', color='green')
    axs1[6].plot(sol.t, beta_er, label='ER', color='blue')
    axs1[6].set_title('Buffering Factors', fontsize=14, fontweight='bold')
    axs1[6].grid(True)
    axs1[6].legend()

    axs1[7].plot(sol.t, dpmca, label='dPMCA', color='cyan', linewidth=2)
    axs1[7].set_title('PMCA Modulation State', fontsize=14, fontweight='bold')
    axs1[7].grid(True)
    axs1[7].legend()

    axs1[8].plot(sol.t, IP3, label='IP3', color='magenta', linewidth=2)
    axs1[8].set_title('IP3 Concentration (mM)', fontsize=14, fontweight='bold')
    axs1[8].grid(True)
    axs1[8].legend()
    ...
    fig2.suptitle('Currents, Fluxes, Gating, and PMCA (Phase 4)', fontsize=16, fontweight='bold')
    ...
    axs2[1].set_title('Calcium Fluxes (mM/ms)', fontsize=14, fontweight='bold')
    axs2[1].grid(True)
    ...
    axs2[2].set_title('Gating Variables (CaL)', fontsize=14, fontweight='bold')
    axs2[2].grid(True)
