...
vector<vector<double>> simulate_process_modified_v2(double g_gap_value, double Ibg_init, double Ikir_coef, double cm, double dx, double K_o, vector<double>& I_app) {
    double dt = 0.001;
    double F = 9.6485e4;
    double R = 8.314e3;
    int loop = 600000;
    ...
    vector<vector<double>> A(loop, vector<double>(7));
    ...
    for (int j = 0; j < loop; j++) {
        ...
        A[j][0] = t;
        A[j][1] = Vm[0];      // First cell
        A[j][2] = Vm[98];     // Cell just before I_app stimulation
        A[j][3] = Vm[99];     // Cell with I_app stimulation
        A[j][4] = Vm[100];    // Cell just after I_app stimulation
        A[j][5] = Vm[101];    // Cell just after stimulation ends
        A[j][6] = Vm[Ng - 1]; // Last cell
    }
...
void generate_training_data_range(int start_index, int end_index, const vector<pair<double, double>>& param_ranges, const string& output_file, int& lines_written, mutex& mtx) {
    ...
    for (int i = start_index; i < end_index; i++) {
        ...
        vector<vector<double>> A = simulate_process_modified_v2(ggap, Ibg_init, Ikir_coef, cm, dx, K_o, I_app_array);

        vector<int> cellids = {0, 98, 99, 100, 101, 199};

        for (int cellid : cellids) {
            file << cellid << "," << ggap << "," << Ikir_coef << "," << cm << "," << K_o << "," << I_app_value;

            for (int j = 0; j < 600000; j++) {
                file << "," << A[j][cellid == 0 ? 1 : cellid - 96];
            }
            file << endl;
        }
...
void generate_training_data(int num_samples, const string& output_file, int num_threads) {
    ...
    // Write header row
    file << "cellid,ggap,Ikir_coef,cm,K_o,I_app";
