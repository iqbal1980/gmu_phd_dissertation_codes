...
##########################################
# De Young-Keizer IP3R parameters (converted to mM and ms^-1)
# Original (μM^-1 s^-1) convert μM->mM by ×1000 and s->ms by /1000
# a1=400 μM^-1 s^-1 => 400 mM^-1 ms^-1 (coincidentally same number after conversion)
a1 = 400.0
a2 = 0.2
a3 = 400.0
a4 = 0.2
...
# IP3 concentration (0.1 μM = 0.0001 mM)
IP3_concentration = 0.0001

# v1 originally s^-1 = 90 s^-1, convert to ms^-1: 90/1000=0.09 ms^-1
v1 = 0.09
...
# IP3R states initial (normalized)
x000_initial = 0.27
x100_initial = 0.039
x010_initial = 0.29
x001_initial = 0.17
...
def calculate_ip3r_flux(x110, Ca_ER, Ca_i):
    Ca_i = max(Ca_i,1e-12)
    Ca_ER = max(Ca_ER,1e-12)
    # IP3R flux = v1*(x110^3)*(Ca_ER - Ca_i)
...
def calculate_pmca_flux(Ca_i, dpmca, params):
    ...
    # divide by 6.6253e5 to convert ions/s to mM/ms: given previously
    u4 = u4/(6.6253e5)
    u5 = u5/(6.6253e5)
...
def calculate_currents(V, Ca_i, atp, dpmca, Ca_ER, x000, x100, x010, x001, x110, x101, x011, x111, params):
    ...
    # Update states outside this function in ODE model
    # IP3R flux:
    J_IP3R = calculate_ip3r_flux(x110, Ca_ER, Ca_i)

    # RyR flux - simplified from original model
    J_RyR = params['conductance_RyR']*(Ca_i/(Ca_i+0.3))*(Ca_ER - Ca_i)

    return (I_Kir61, I_TRPC1, I_CaCC, I_CaL, I_leak, d_inf, f_inf, I_PMCA, J_SERCA,
            J_ER_leak, I_NCX, J_IP3R, J_RyR,
...
def model(t, y, params):
    ...
    (I_Kir61, I_TRPC1, I_CaCC, I_CaL, I_leak, d_inf, f_inf, I_PMCA,
     ...
     dx000_dt, dx100_dt, dx010_dt, dx001_dt, dx110_dt, dx101_dt, dx011_dt, dx111_dt) = \
        calculate_currents(V, Ca_i, atp, dpmca, Ca_ER, x000, x100, x010, x001, x110, x101, x011, x111, params)
...
def plot_results_with_two_figures(sol, params):
    ...
    # Recompute currents for plotting
    ...
    fig1.suptitle('Cell Dynamics - Part 1 (Phase 1)', fontsize=16, fontweight='bold')
    ...
    # Intracellular Ca regular scale
    ...
    # IP3R states: plot x110 as main open state
    ...
    axs1[3].set_title('IP3R1 State x110', fontsize=14, fontweight='bold')
    
    axs1[3].legend()
    axs1[3].grid(True)

    # Intracellular Ca log scale
    ...
    fig2.suptitle('Cell Dynamics - Part 2 (Phase 1)', fontsize=16, fontweight='bold')
