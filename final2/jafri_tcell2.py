...
class TCellModel:
    ...
    def run_simulation(self, total_time, dt):
        ...
        solver.set_integrator('lsoda', atol=1e-8, rtol=1e-8, max_step=dt)
        ...
        for i in range(num_steps):
            if solver.successful():
                ...
                # Extract results
                results['cai'][i] = state[0]
                results['caer'][i] = state[1]
                results['cam'][i] = state[2]
                results['cas'][i] = state[8]
                results['V'][i] = state[9]
                results['aip3'][i] = state[3]
                results['dag'][i] = state[4]
                results['x100'][i] = state[5]
                results['x110'][i] = state[6]
                
                nfat_start = 25
                nfkb_start = nfat_start + len(self.nfat.x)
                jnk_start = nfkb_start + len(self.nfkb.x)
                
                results['nfat_active'][i] = self.nfat.get_active_fraction(state[nfat_start:nfkb_start])
                results['nfkb_active'][i] = self.nfkb.get_active_fraction(state[nfkb_start:jnk_start])
                results['jnk_active'][i] = self.jnk.get_active_fraction(state[jnk_start:])
            else:
                print(f"Solver failed at t={solver.t}")
                break
...
def run_simulation(total_time=2400, dt=0.001):
    model = TCellModel()
