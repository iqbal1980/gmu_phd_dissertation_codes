...
# Hardcoded ion concentrations (unchanged)
...
# Time simulation parameters (unchanged)
...
def calculate_ip3r_flux(x110, Ca_ER, Ca_i):
    ...
    # IP3R flux depends on the cube of x110 state (three subunits must be in open state)
...
def calculate_currents(V, Ca_i, atp, dpmca, Ca_ER, x110, params):
   ...
   # IP3R flux using De Young-Keizer model
...
def model(t, y, params):
   ...
   # Calculate all currents and fluxes using the De Young-Keizer x110 state
...
def plot_results_with_two_figures(sol, params):
   ...
   # Evaluate currents at each time point with IP3R states
