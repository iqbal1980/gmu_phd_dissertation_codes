...
Km_serca = 0.5
leak_rate_er = 0.05
IP3_concentration = 0.1  # Example IP3 concentration
k_ncx = 0.001
...
def calculate_currents(V, Ca_i, atp, dpmca, params):
    ...
    #TODO:This goes in Ca equation not membrane equation
    # SERCA pump flux
    I_SERCA = params['k_serca'] * (Ca_i**2) / (Ca_i**2 + params['Km_serca']**2)

    # ER Leak
    I_ER_leak = params['leak_rate_er'] * (params['Ca_ER'] - Ca_i)
    ...
    #TODO:Include Fluxes <---***
    # IP3R currents
    #TODO: Change this channel is not usually in the membrane
    I_IP3R1 = params['conductance_IP3R1'] * (V - params['reversal_potential_Ca']) * (Ca_i / (Ca_i + 0.2)) * (params['IP3_concentration'] / (params['IP3_concentration'] + 0.1))
    I_IP3R2 = params['conductance_IP3R2'] * (V - params['reversal_potential_Ca']) * (Ca_i / (Ca_i + 0.15)) * (params['IP3_concentration'] / (params['IP3_concentration'] + 0.1))

    # Ryanodine receptor (RyR) current
    #TODO: Change this not in membrane
    I_RyR = params['conductance_RyR'] * (V - params['reversal_potential_Ca']) * (Ca_i / (Ca_i + 0.3))

    return I_Kir61, I_TRPC1, I_CaCC, I_CaL, I_leak, d_inf, f_inf, I_PMCA, I_SERCA, I_ER_leak, I_NCX, I_IP3R1, I_IP3R2, I_RyR

# ODE model function
def model(t, y, params):
    V, Ca_i, atp, dpmca = y
    I_Kir61, I_TRPC1, I_CaCC, I_CaL, I_leak, _, _, I_PMCA, I_SERCA, I_ER_leak, I_NCX, I_IP3R1, I_IP3R2, I_RyR = calculate_currents(V, Ca_i, atp, dpmca, params)
    ...
    Ca_efflux = params['calcium_extrusion_rate'] * (Ca_i - params['resting_calcium']) #TODO:Check Ca_influx Ca_efflux
    #TODO: Revise this needs to be flux on both sides<---*** Not fluxes in one side and currents in the other
    dCa_dt = Ca_influx - Ca_efflux - I_SERCA + I_ER_leak + I_IP3R1 + I_IP3R2 + I_RyR
    ...
    #we need to have dynamic ODE for ER

    if t % 10 == 0:  # Print every 10 ms
        print(f"Time: {t:.2f} ms, V: {V:.2f} mV, Ca_i: {Ca_i:.6f} mM")
        print(f"I_Kir61: {I_Kir61:.4f}, I_TRPC1: {I_TRPC1:.4f}, I_CaCC: {I_CaCC:.4f}, I_CaL: {I_CaL:.4f}, I_leak: {I_leak:.4f}, I_PMCA: {I_PMCA:.4f}, I_SERCA: {I_SERCA:.4f}, I_ER_leak: {I_ER_leak:.4f}, I_NCX: {I_NCX:.4f}, I_IP3R1: {I_IP3R1:.4f}, I_IP3R2: {I_IP3R2:.4f}, I_RyR: {I_RyR:.4f}")
...
def plot_results_with_two_figures(sol, params):
    ...
    I_Kir61, I_TRPC1, I_CaCC, I_CaL, I_leak, d_inf, f_inf, I_PMCA, I_SERCA, I_ER_leak, I_NCX, I_IP3R1, I_IP3R2, I_RyR = [
        np.zeros_like(sol.t) for _ in range(14)
    ]

    # Evaluate currents at each time point
    for i, (V, Ca_i, atp, dpmca) in enumerate(zip(sol.y[0], sol.y[1], sol.y[2], sol.y[3])):
        (
            I_Kir61[i], I_TRPC1[i], I_CaCC[i], I_CaL[i], I_leak[i], d_inf[i], f_inf[i], I_PMCA[i],
            I_SERCA[i], I_ER_leak[i], I_NCX[i], I_IP3R1[i], I_IP3R2[i], I_RyR[i]
        ) = calculate_currents(V, Ca_i, atp, dpmca, params)
    ...
    axs2[0].plot(sol.t, I_SERCA, label='I_SERCA', alpha=0.8, linewidth=2, color='purple')
    axs2[0].plot(sol.t, I_ER_leak, label='I_ER_leak', alpha=0.8, linewidth=2, color='cyan')
    axs2[0].plot(sol.t, I_NCX, label='I_NCX', alpha=0.8, linewidth=2, color='magenta')
    axs2[0].plot(sol.t, I_IP3R1, label='I_IP3R1', alpha=0.8, linewidth=2, color='pink')
    axs2[0].plot(sol.t, I_IP3R2, label='I_IP3R2', alpha=0.8, linewidth=2, color='orange')
    axs2[0].plot(sol.t, I_RyR, label='I_RyR', alpha=0.8, linewidth=2, color='brown')
...
params = {
    ...
    'calcium_activation_threshold_CaCC': calcium_activation_threshold_CaCC,  # Ensure this is present
    ...
    'Ca_ER': 0.1  #TODO: Example ER calcium concentration (mM) <---**** 500micro molar
}
