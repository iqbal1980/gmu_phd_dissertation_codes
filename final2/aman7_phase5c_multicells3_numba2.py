...
###############################################################################
# Tweak how often we do a debug print from the single-cell model function
###############################################################################
DEBUG_PRINT_DT = 1.0  # in ms, how often to print [model DEBUG] t=...

###############################################################################
# PHASE 5 SINGLE-CELL MODEL: GLOBAL CONSTANTS, ETC.
###############################################################################
...
x011_initial = 0.18
x111_initial = 0.0035
tot1 = (x000_initial + x100_initial + x010_initial + x001_initial +
...
tot2 = (x0002_initial + x1002_initial + x0102_initial + x0012_initial +
...
###############################################################################
# Numba JITted Helper Functions (object mode)
###############################################################################
@jit(forceobj=True)
...
def calculate_currents(V, Ca_i, atp, dpmca, Ca_ER, x110, x1102, IP3, params):
    ...
    d_inf = 1.0/(1.0+ np.exp(-(V- params['activation_midpoint_CaL'])/
                             params['activation_slope_CaL']))
    f_inf = (1.0/(1.0+ np.exp((V- params['inactivation_midpoint_CaL'])/
                             params['inactivation_slope_CaL']))
             + params['amplitude_factor_CaL']/
               (1.0+np.exp((params['voltage_shift_CaL'] - V)/
    ...
    I_Kir61= (params['conductance_Kir61']*atp*np.sqrt(params['K_out']/params['reference_K'])
              *(V-params['reversal_potential_K'])
              /(1.0+ np.exp((V-params['reversal_potential_K']-
                             params['voltage_shift_Kir61'])/
...
###############################################################################
# MAKE A SINGLE-CELL MODEL FUNCTION THAT PRINTS EVERY debug_dt
###############################################################################
def make_model_func(debug_dt):
    """
    Returns a Numba-jitted 'model' function that prints
    the current solver time every debug_dt ms.
    """
    last_print = [float('-inf')]  # local mutable state

    @jit(forceobj=True)
    def model(t, y, params):
        # If enough time has passed, do a debug print:
        ...
        (dx0002, dx1002, dx0102, dx0012,
         dx1102, dx1012, dx0112, dx1112) = calculate_ip3r_states(
            Ca_i, IP3, x0002, x1002, x0102, x0012,
...
def run_simulation(params):
    print("[DEBUG] Starting single-cell simulation with final time =",
          params['simulation_duration'],
          "ms using BDF solver...")

    # Build the model function with debug prints every DEBUG_PRINT_DT
    model_func = make_model_func(DEBUG_PRINT_DT)
    ...
    print(f"[DEBUG] Single-cell simulation completed in {end_time - start_time:.4f} s.")
    return sol

def plot_results_with_two_figures(sol, params):
    print("[DEBUG] Plotting single-cell results (brief version).")
    plt.figure()
    plt.plot(sol.t, sol.y[0], label="Vm")
    plt.xlabel("Time (ms)")
    plt.ylabel("Voltage (mV)")
    plt.title("Single-Cell Membrane Potential (Phase 5, BDF solver)")
    plt.legend()
    plt.show()
...
def multi_cell_model(t, Y, params_multi):
    ...
    # We'll reuse the single-cell model_func but no debug prints for each cell/time
    # to avoid huge spam. Or you can build a separate "model_func_multi" if you want.
    # For simplicity we just replicate the single-cell approach minus prints.
    # Typically you'd define a "model_func_multi(t, y, cellparams)" outside, but let's inline it.
    ...
    for i in range(Ng):
        ...
        # We'll call the same "model_func" but we won't do debug prints
        # because that function we built specifically prints. We can do a simpler
        # approach here or copy/paste the ODE code. For brevity, just call a separate function:
        dyi = single_cell_model_without_debug(t, y_i, cellparams)
...
@jit(forceobj=True)
def single_cell_model_without_debug(t, y, params):
    """
    Exactly like 'model_func' but no debug prints or closure usage.
    This is so multi_cell_model can call it for each cell in object mode.
    """
    (V, Ca_i, atp, dpmca, Ca_ER,
     x000, x100, x010, x001, x110, x101, x011, x111,
     IP3, Ca_m,
...
def run_multi_cell_simulation(params_multi, t_span=(0,600), dt=1.0):
    print("[DEBUG] Starting multi-cell simulation with final time =",
          t_span[1], "ms using BDF solver...")
...
if __name__=="__main__":
    ...
    # Prepare single-cell param dictionary
    params_cell = {
        'K_out': K_out,
        'K_in':  K_in,
        'Ca_out':Ca_out,
    ...
    plt.xlabel("Time (ms)")
    plt.ylabel("Voltage (mV)")
    plt.title("Multi-cell Pericyte Model with BDF + Stimulus")
    plt.legend()
    plt.show()
