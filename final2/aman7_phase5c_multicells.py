...
#                            SINGLE-CELL MODEL
###############################################################################
# Below is your single-cell model (“Phase 5 with IP3R2, mitochondrial flux, etc.”),
# including all the parameters, initial conditions, and the ODE function `model(...)`.
...
# ---------- Helper functions for single-cell model (Nernst, buffers, etc.) ---------- #
...
def calculate_currents(V, Ca_i, atp, dpmca, Ca_ER, x110, x1102, IP3, params):
    ...
    I_Kir61 = (params['conductance_Kir61']*atp*np.sqrt(params['K_out']/params['reference_K']) *
               ...
               (1.0 + np.exp((V - params['reversal_potential_K'] - params['voltage_shift_Kir61'])/params['voltage_slope_Kir61'])))
...
def model(t, y, params):
    """
    Single-cell ODE system with your updated channels, IP3R1, IP3R2, SERCA, RyR, Mito flux, etc.
    ...
    # dV/dt from net membrane current
    dV_dt = (-I_Kir61 - I_TRPC1 - I_CaCC - I_CaL - I_leak - I_PMCA - I_NCX)/membrane_capacitance
    
    # Ca influx from I_CaL
    Ca_influx = -I_CaL/(2*z*faraday_constant*cell_volume)  # sign adjusted
    # Ca efflux from PMCA + NCX
    Ca_efflux = (-I_PMCA - I_NCX)/(2*z*faraday_constant*cell_volume)
...
def run_simulation(params):
    """
    Run the single-cell simulation (if you want single-cell only).
    """
...
def plot_results_with_two_figures(sol, params):
    """
    Plot single-cell simulation results (as in your original code).
    """
    V      = sol.y[0]
    Ca_i   = sol.y[1]
    dpmca  = sol.y[3]
    ...
    # Recompute fluxes and gating for plotting
    ...
    for i, t_val in enumerate(sol.t):
        # For plotting, pass updated states to calculate fluxes
    ...
    # Plot: as in your original code, 2 figures
    # (Figure 1: states, Figure 2: currents/fluxes)
...
###############################################################################
#                 MULTI-CELL WRAPPER WITH GAP-JUNCTIONS
###############################################################################
def multi_cell_model(t, Y, params_multi):
    """
    Extends the single-cell model to Ng cells coupled via gap junctions.
    
    Y is a long 1D array of length (nvar * Ng), where nvar=23 for your model.
    For cell i, the subvector is Y[i*nvar : (i+1)*nvar].
    
    We'll parse out each cell's states, call the single-cell ODE `model(...)`,
    and then add gap-junction currents to dV/dt.
    """
    ...
    for i in range(Ng):
        ...
        dyi = model(t, y_i, cellparams)  # calls your single-cell ODE
        
        # The first element in dyi is dV/dt; we need to add gap-junction coupling
        dVdt_single = dyi[0]
        
        # Get V_i
        V_i = y_i[0]
        
        # Identify neighbors: for 1D chain, left = i-1, right = i+1
        # Simple boundary: no flux if i==0 or i==Ng-1
        if i == 0:
            V_left = V_i
        else:
            V_left = Y[(i-1)*nvar + 0]
        ...
        # Gap-junction current (discrete Laplacian)
        I_gj = (g_gap / (dx**2)) * (V_left + V_right - 2.0*V_i)
        
        # Convert to dV/dt by dividing by cm
...
def run_multi_cell_simulation(params_multi, t_span=(0,600), dt=0.1):
    """
    Runs the multi-cell simulation using solve_ivp.
    """
    Ng    = params_multi['Ng']
    nvar  = params_multi['nvar']  # 23
    ...
    # Build multi-cell y0 by replicating single-cell initial condition
...
if __name__ == "__main__":
    # 1) Prepare single-cell params for your model
    params_cell = {
        'K_out': K_out,
        'K_in': K_in,
        'Ca_out': Ca_out,
    ...
    # Compute reversal potentials for K, Ca, Na, Cl and update
    ...
    # Example: plot membrane potential of each cell
    plt.figure()
    for i_cell in range(params_multi['Ng']):
        idxV = i_cell*params_multi['nvar'] + 0  # V is the 0th state
        plt.plot(t_multi, Y_multi[idxV, :], label=f'Cell {i_cell}')
    plt.xlabel("Time (ms)")
    plt.ylabel("Voltage (mV)")
    plt.title("Multi-cell Pericyte Model (Gap-Junction Coupling)")
    plt.legend()
    plt.show()
