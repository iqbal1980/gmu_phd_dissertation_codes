...
V_norm = 100  # mV
Ca_norm = 0.001  # mM (adjusted for more realistic calcium scaling)
K_norm = 140  # mM
Na_norm = 140  # mM
Cl_norm = 120  # mM
ATP_norm = 10  # mM
...
def model(t, y, params):
    ...
    # Enforce stricter bounds on variables
    V = np.clip(V, -100, 50)
    Ca_i = np.clip(Ca_i, 1e-7, 1e-3)
    K_i = np.clip(K_i, 100, 160)
    Na_i = np.clip(Na_i, 5, 20)
    Cl_i = np.clip(Cl_i, 4, 40)
    ...
    I_CaL = g_CaL * (V - E_Ca)
    I_CaCC = g_CaCC * (Ca_i / (Ca_i + 0.5e-3)) * (V - E_Cl)
    I_leak = g_leak * (V - E_K)
    ...
    J_ip3r = k_ip3r * (IP3 / (IP3 + k_ip3r)) * (Ca_i / (Ca_i + 0.3e-3)) * (Ca_er - Ca_i)
...
problem = {
    ...
    'bounds': [[0.01, 0.5], [0.0001, 0.01], [0.0001, 0.01], [0.001, 0.1], [0.05, 1.0], [0.05, 0.5]]
}
...
def run_and_plot_simulation(params):
    ...
    ax2.plot(t, sol[:, 1] * Ca_norm * 1e3, label='Ca2+ (μM)')
...
def adjust_parameters(target_voltage, target_calcium, param_values_valid, Si):
    ...
    for i in range(5):  # Perform 5 iterations of adjustment
        ...
        print(f"Current calcium: {result[1]*1e6:.6f} μM, Target: {target_calcium*1e6:.6f} μM, Error: {calcium_error*1e6:.6f} μM")
