...
#https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6583802/
#https://www.ninds.nih.gov/health-information/disorders/cadasil
#https://www.ncbi.nlm.nih.gov/pmc/articles/PMC10378680/
...
def ode_system(t, y, g_gap, Ibg_init, Ikir_coef, cm, dx, K_o, I_app_val, stimulation_time_start, stimulation_time_end, activated_cell_index):
    ...
    Vm = y
    I_bg = Ibg_init * (Vm + 30)
    E_K = (R * 293 / F) * safe_log(np.maximum(K_o / 150, 1e-20))
...
def generate_random_params():
    ...
    y0_init = -33
    Ng = 200
    activated_cell_index = np.random.randint(85, 115)
...
def physics_loss(x, y):
    ...
    F = 9.6485e4
    R = 8.314e3
    E_K = (R * 293 / F) * torch.log(torch.clamp(K_o / 150, min=1e-20))  
