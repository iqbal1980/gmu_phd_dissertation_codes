...
# Define the parameter space for sensitivity analysis
problem = {
    'num_vars': 10,
...
def perform_sensitivity_analysis():
    ...
    for i, sample in enumerate(param_values):
        ...
        # Run simulation and collect output data (final intracellular calcium concentration)
        try:
            sol = run_simulation(params)
            final_calcium_concentration = sol.y[1][-1]  # Final value of intracellular Ca2+
        except:
            final_calcium_concentration = np.nan  # Handle simulation failures

        Y.append(final_calcium_concentration)
