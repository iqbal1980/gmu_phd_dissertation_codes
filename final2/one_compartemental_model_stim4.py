...
def model(t, y):
    ...
    # Membrane potential
    dV_dt = (-I_Kir61 - I_TRPC1 - I_CaCC - I_ASIC2 + I_stim(t)) / Cm
...
axs[0, 1].set_ylabel('ATP')
axs[1, 0].plot(sol.t, [I_stim(ti) for ti in sol.t])
axs[1, 0].set_ylabel('Stimulation Current (pA)')
for ax in axs.flat:
