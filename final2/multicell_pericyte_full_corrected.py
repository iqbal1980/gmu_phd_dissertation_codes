...
"""
multicell_pericyte_full_corrected.py

Demonstration of a "corrected" multi-cell PDE-like approach for a pericyte model
with 23 states (IP3R1/IP3R2, mitochondria, PMCA, SERCA, RyR, etc.). The cells
are coupled only through gap junction (GJ) in the membrane potential (V).

We also apply a patch-clamp style current (I_app) to one cell in the chain to
simulate a "stimulator/responder" experiment, similar to the approach used in
papers by Dr. Longden et al.
...
###############################################################################
# Index definitions for each of the 23 states in single-cell ODE
###############################################################################
STATE_V       = 0
STATE_CAI     = 1
STATE_ATP     = 2
...
###############################################################################
# Single-Cell ODE: "23-state" detailed pericyte model
###############################################################################
def single_cell_rhs_full(t, y_cell, p, cell_index):
    """
    single_cell_rhs_full(t, y_cell, p, cell_index)
      -> returns dy/dt for one pericyte with 23 states.

    y_cell has length 23:
      [V, Ca_i, atp, dpmca, Ca_ER,
       x000, x100, x010, x001, x110, x101, x011, x111,
       IP3, Ca_m,
       x0002, x1002, x0102, x0012, x1102, x1012, x0112, x1112]

    We also add a patch-clamp style current if (cell_index == p['stim_cell'])
    and t is in [p['stim_start'], p['stim_end']].
    """
    # Unpack states
    V      = y_cell[STATE_V]
    ...
    dx000,dx100,dx010,dx001,dx110,dx101,dx011,dx111 = \
        calc_ip3r_states(Cai, IP3, x000, x100, x010, x001, x110, x101, x011, x111)
    # IP3R2 states
    dx0002,dx1002,dx0102,dx0012,dx1102,dx1012,dx0112,dx1112 = \
        calc_ip3r_states(Cai, IP3, x0002, x1002, x0102, x0012, x1102, x1012, x0112, x1112)
    ...
    I_Kir61 = (
        ...
        * (V - p['reversal_potential_K'])
        / (1.0 + math.exp(
            (V - p['reversal_potential_K'] - p['voltage_shift_Kir61'])
             / p['voltage_slope_Kir61']
          ))
    )
    # TRPC1
    ...
    def CaL_gating(Vv):
        ...
        f_tmp= 1.0/(1.0 + math.exp(
                     (Vv - p['inactivation_midpoint_CaL'])
                     / p['inactivation_slope_CaL']))
    ...
    # Patch clamp current?
    I_app= 0.0
    # If we only clamp the 'stim_cell' in the time window
    ...
    # Return the derivatives in a 23-length array
    dy= np.zeros(23, dtype=float)
    dy[STATE_V]      = dVdt
    dy[STATE_CAI]    = dCa_dt
    dy[STATE_ATP]    = 0.0   # not modeled here
...
###############################################################################
# PDE-like approach with N cells, each having 23 states, gap-junction on V
###############################################################################
def multicell_rhs(t, Y, p):
    """
    We treat N cells in a chain. Each has 23 states.
    We'll do gap-junction coupling only in the membrane potential (STATE_V).

    Y is length N*23. For cell i, states range from [i*nvar ... i*nvar+22].
    """
    N= p['N_cells']
    nvar= 23
    g_gap= p['g_gap']
    ...
    for i in range(N):
        ...
        # Single-cell ODE for cell i
        dy_i= single_cell_rhs_full(t, y_i, p, cell_index=i)

        # Then we add a gap-junction term to V:
        V_i= y_i[STATE_V]
        if i==0:
            V_left= V_i
        else:
        ...
        # I_gj for cell i (second-difference approach)
...
def run_multicell_sim(p):
    """
    Build initial condition for each cell, integrate from t=0 to t_final.
    """
    N= p['N_cells']
    nvar= 23
    ...
    def init_single_cell():
        ...
        # Basic single-cell initial conditions
        arr[STATE_V]      = -70.0
        arr[STATE_CAI]    = 1e-4
        arr[STATE_ATP]    = 4.4
        arr[STATE_DPMCA]  = 1.0
    ...
    sol= solve_ivp(rhs_wrap, t_span, bigY0, t_eval=t_eval,
                   method='RK45', rtol=1e-6, atol=1e-9)
    return sol

def plot_multicell(sol, p):
    """
    Basic plotting: We'll show
      - V(t) for all cells
      - Ca_i(t) for all cells
    """
    t= sol.t
    Y= sol.y
    N= p['N_cells']
...
if __name__=="__main__":
    ...
    p= {
        ...
        # External/ internal ionic concentrations
        'K_out':6.26,
        'Ca_out':2.0,
        'Na_in':15.38,

        # Add the missing reversal potentials
        ...
        # PDE / multicell parameters
        'N_cells':5,       # number of cells
        'g_gap':0.02,      # gap-junction conductance, adjust as needed
        'dx':1.0,          # spacing
        't_final':600.0,   # ms

        # Patch clamp
        'stim_cell':0,     # which cell gets the clamp
        'I_app_val':70.0,  # pA
        'stim_start':100.0,# ms
        'stim_end':400.0   # ms
    }
