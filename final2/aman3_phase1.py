...
# Original from T-cell code (in μM and s)
# Convert to mM and ms:
# d1 = 0.13 μM → 0.00013 mM
# d2 = 1.049 μM → 0.001049 mM
# d3 = 0.9434 μM → 0.0009434 mM
# d4 = 0.1445 μM → 0.0001445 mM
# d5 = 82.34e-3 μM = 0.08234 μM → 0.00008234 mM
...
# a-parameters (from μM^-1 s^-1 to mM^-1 ms^-1)
# a1 = 400 μM^-1 s^-1 → 400 mM^-1 ms^-1
# a2 = 0.2 μM^-1 s^-1 → 0.2 mM^-1 ms^-1
# a3 = 400 μM^-1 s^-1 → 400 mM^-1 ms^-1
# a4 = 0.2 μM^-1 s^-1 → 0.2 mM^-1 ms^-1
# a5 = 20 μM^-1 s^-1 → 20 mM^-1 ms^-1
...
# Normalize IP3R states so they sum to 1
x000_initial = 0.27
x100_initial = 0.039
x010_initial = 0.29
x001_initial = 0.17
...
def calculate_ip3r_states(Ca_i, x000, x100, x010, x001, x110, x101, x011, x111):
    # IP3R state transitions with converted parameters
...
def calculate_currents(V, Ca_i, atp, dpmca, Ca_ER, x110, params):
    ...
    # For plotting consistency, return J_IP3R2 as 0 (not used here)
...
def plot_results_with_two_figures(sol, params):
    ...
    axs1[4].plot(sol.t, sol.y[1], color='green', linewidth=2)
    axs1[4].set_yscale('log')
    axs1[4].set_title('Intracellular Calcium Concentration (Log Scale)', fontsize=14, fontweight='bold')
    axs1[4].set_ylabel('Intracellular\nCa2+ (mM)', fontsize=12, fontweight='bold')
    axs1[4].grid(True)
...
params = {
    ...
    'reversal_potential_K': None,  # to be filled in
