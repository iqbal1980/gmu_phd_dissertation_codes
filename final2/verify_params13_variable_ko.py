...
def HK_deltas_vstim_vresponse_graph_modified_v2(ggap, Ibg_init, Ikir_coef, cm, dx, K_o_left, K_o_right):
    ...
    np.savetxt(f"a{ggapval}.csv", A, delimiter=",")
    
    # New line to reduce the number of rows by a factor of 10
    xy_combined_reduced = xy_combined[::10, :]  # This selects every 10th row

    # Save the reduced data to a CSV file
    np.savetxt(f"xy_combined_reduced_data_{ggapval}.csv", xy_combined_reduced, delimiter=",")
