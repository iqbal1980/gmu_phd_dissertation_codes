...
# Mitochondrial parameters (Phase 5, from T-cell style)
Vmito = Vcyto*0.08
Pmito = 2.776e-14
psi = 160.0
alphm = 0.2
...
Ca_ER_initial = 0.5
IP3_initial = 0.1
Ca_m_initial = 0.0001  # mitochondrial Ca²⁺ initial guess
...
def model(t, y, params):
    ...
    if abs(t-round(t))<1e-9 and round(t)%10==0:
        ...
        print(f"Mito fluxes: J_uni={J_uni:.6f}, J_nc={J_nc:.6f}")

    return [dV_dt, dCa_dt, datp_dt, ddpmca_dt, dCa_ER_dt,
            dx000_dt, dx100_dt, dx010_dt, dx001_dt, dx110_dt, dx101_dt, dx011_dt, dx111_dt, dIP3_dt, dCa_m_dt]
...
def run_simulation(params):
    ...
    y0 = [params['initial_voltage'], params['initial_calcium'], params['initial_atp'], params['initial_dpmca'],
          ...
          x101_initial, x011_initial, x111_initial, IP3_initial, Ca_m_initial]
...
def plot_results_with_two_figures(sol, params):
    ...
    for i in range(len(sol.t)):
        ...
        (I_Kir61[i], I_TRPC1[i], I_CaCC[i], I_CaL[i], I_leak[i], d_inf[i], f_inf[i], I_PMCA[i],
         J_SERCA[i], J_ER_leak[i], I_NCX[i], J_IP3R[i], J_RyR[i]) = calculate_currents(V[i], Ca_i[i], 
                                                                                      params['initial_atp'], dpmca[i],
                                                                                      Ca_ER[i], x110[i], IP3[i], params)

    # Figure 1
    fig1, axs1 = plt.subplots(8, 1, figsize=(12, 40))
    fig1.suptitle('Cell Dynamics with Mitochondrial Dynamics', fontsize=16, fontweight='bold')
    ...
    axs1[7].plot(sol.t, Ca_m, label='Ca_m', color='brown')
    axs1[7].set_title('Mitochondrial Ca²⁺ (mM)', fontsize=14, fontweight='bold')
    axs1[7].grid(True)
    ...
    fig2.suptitle('Currents, Fluxes, Gating, and PMCA', fontsize=16, fontweight='bold')
