...
def physics_based_loss(model, inputs, targets):
    params = model(inputs).detach().cpu().numpy()  # Move the tensor to CPU before converting to NumPy
    ...
    return torch.tensor(loss, requires_grad=True).to(device)  # Move the loss tensor to the GPU
...
def main():
    ...
    model = PerocyteModel().to(device)  # Create the model on the GPU
    ...
    test_inputs = torch.rand(100, 4).to(device)  # Create the test inputs on the GPU
    with torch.no_grad():
