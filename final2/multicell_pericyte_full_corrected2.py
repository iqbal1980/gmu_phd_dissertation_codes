...
"""
multicell_pericyte_full_corrected_mirror.py

Runs the "corrected" 23-state pericyte PDE-like model TWICE:
1) Once with I_app_val = +70 pA  ("sail" shape)
2) Once with I_app_val = -70 pA  ("mirror" shape)

Then plots the results for each run. This won't replicate the simpler
"Kir-only" code's perfect mirror, but allows direct sign-flip comparison.
...
###############################################################################
# Index definitions for each of the 23 states
###############################################################################
STATE_V       = 0
STATE_CAI     = 1
STATE_ATP     = 2
...
def plot_multicell(sol, p, label_suffix=""):
    t= sol.t
    Y= sol.y
    N= p['N_cells']
    nvar= 23
    ...
    plt.title(
        f"Voltage, N={N}, Gap={p['g_gap']}, I_app={p['I_app_val']} {label_suffix}"
    )
    plt.xlabel("Time (ms)")
    plt.ylabel("Vm (mV)")
    ...
    plt.title(
        f"Cytosolic Ca, N={N}, Gap={p['g_gap']}, I_app={p['I_app_val']} {label_suffix}"
    )
    plt.xlabel("Time (ms)")
    plt.ylabel("Ca_i (mM)")
    plt.legend(bbox_to_anchor=(1.05,1), loc="upper left")
...
if __name__=="__main__":
    ...
    plot_multicell(sol2, p, label_suffix="(I_app=-70)")
