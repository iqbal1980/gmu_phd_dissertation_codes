...
calcium_out = 0.05  # Extracellular calcium concentration
calcium_in = 0.00001  # Intracellular calcium concentration
...
conductance_TRPC1 = 0  # Set to 0 as per Dr. Jafri's comment
conductance_CaCC = 0.001  # Reduced to make it small at rest
conductance_CaL = 0.00001#0.001  # Reduced to balance with KATP at rest
#conductance_leak = 0.001  # Added leak conductance for balancing
...
calcium_extrusion_rate = 0.1#0.01  # Rate constant for calcium extrusion (1/ms)
...
def calculate_currents(V, Ca_i, atp):
    ...
    I_leak = 0  # Leak current set to 0
...
def model(t, y):
    ...
    datp_dt = 0  # ATP concentration is now constant
...
def save_to_csv(filename, sol):
    t = sol.t
    V, Ca_i, atp = sol.y
    I_Kir61, I_TRPC1, I_CaCC, I_CaL, _, d_inf, f_inf = calculate_currents(V, Ca_i, atp)
...
I_Kir61, I_TRPC1, I_CaCC, I_CaL, _, d_inf, f_inf = calculate_currents(sol.y[0], sol.y[1], sol.y[2])
...
total_inward = I_CaCC[rest_index] + I_CaL[rest_index]
total_outward = I_Kir61[rest_index]
balance = total_inward + total_outward

print(f"\nCurrent balance at rest:")
print(f"Total inward current (I_CaCC + I_CaL): {total_inward:.4f} pA")
print(f"Total outward current (I_Kir61): {total_outward:.4f} pA")
